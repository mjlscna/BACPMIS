...
<div class="flex flex-col {{ $colspan }}">
    ...
    @else
    <input type="{{ $type }}" id="{{ $id }}" wire:model="{{ $model }}" {{ $maxlength ? "maxlength={$maxlength}" : '' }}
        readonly {!! $attributes->merge([
    'class' =>
    ($inputAttributes ?: 'mt-1 block w-full px-3 py-2 text-sm border border-gray-300 rounded-md bg-gray-100
    cursor-not-allowed text-gray-700') . ' ' . $textAlignClass,
    ]) !!} />
    @endif
</div>
