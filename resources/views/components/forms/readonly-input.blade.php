...
<div class="flex flex-col {{ $colspan }}">
    @if ($label)
        <label for="{{ $id }}"
            class="block text-sm font-medium {{ $viewOnly ? 'text-gray-800 dark:text-neutral-400' : 'text-gray-700 dark:text-white' }} mb-1">
    ...
    @else
        <input type="{{ $type }}" id="{{ $id }}" wire:model="{{ $model }}"
            {{ $maxlength ? "maxlength={$maxlength}" : '' }} readonly {!! $attributes->merge([
                'class' =>
                    ($inputAttributes ?:
                        'mt-1 block w-full px-3 py-2 text-sm border border-gray-300 rounded-md bg-gray-100
                                                                            cursor-not-allowed text-gray-700 dark:text-white dark:bg-neutral-700') .
                    ' ' .
                    $textAlignClass,
            ]) !!} />
    @endif
