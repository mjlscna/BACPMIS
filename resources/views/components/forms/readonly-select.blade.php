...
@php
    ...
    $fieldKey = str($model)->replace('form.', '')->toString();
    $rawValue = $value ?? data_get($form, $fieldKey);
    $resolvedLabel = $options[$rawValue] ?? ($rawValue ?? '—');
@endphp
...
<div class="flex flex-col {{ $colspan }}">
    ...
    @if ($viewOnly)
        <div class="text-sm font-semibold text-gray-900 dark:text-white ms-0">
            {{ $resolvedLabel }}
        </div>
    @else
        <input type="text" id="{{ $id }}" wire:model="{{ $model }}" value="{{ $resolvedLabel }}"
