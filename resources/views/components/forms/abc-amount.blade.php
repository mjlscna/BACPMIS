...
<div class="flex flex-col {{ $colspan }}" x-data="{ total: {{ $total }} }" x-init="// Watch Livewire 'form.items' array and recalc total when any amount changes
$watch('$wire.form.items', (items) => {
    ...
    @if ($viewOnly)
        <div class="text-sm font-semibold text-gray-900 dark:text-white ms-3">
            ₱ <span x-text="total.toLocaleString('en-US', { minimumFractionDigits: 2 })"></span>
        </div>
    @elseif($disabled)
        <div class="relative">
            <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500">₱</span>
            <input type="text" id="{{ $id }}" x-model="total" readonly
