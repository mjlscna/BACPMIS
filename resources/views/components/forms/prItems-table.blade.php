    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 rounded-xl">
            <thead class="bg-gray-50">
                <tr>
                    <th class="px-3 md:px-6 py-2 md:py-3 text-center text-[10px] md:text-xs font-medium text-gray-500 uppercase whitespace-nowrap w-16 md:w-20">
                        Item No
                    </th>
                    <th class="px-3 md:px-6 py-2 md:py-3 text-left text-[10px] md:text-xs font-medium text-gray-500 uppercase">
                        Description
                    </th>
                    @unless($viewOnly)
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($items as $index => $item)
                    <tr wire:key="item-{{ $offset + $index }}">
                        <td class="px-3 md:px-6 py-2 md:py-4 text-center text-xs md:text-sm text-gray-800">
                            @if($viewOnly)
                                <div class="text-gray-900">{{ $item['item_no'] ?? '-' }}</div>
                            @else
                                <input type="text"
                                    class="border border-gray-300 rounded-lg px-2 py-1 w-14 md:w-20 text-center text-xs md:text-sm focus:ring-emerald-500 focus:border-emerald-500"
                                    placeholder="#"
                                    wire:model.defer="{{ $model }}.{{ $offset + $index }}.item_no">
                            @endif
                        </td>
                        <td class="px-3 md:px-6 py-2 md:py-4 text-xs md:text-sm text-gray-800">
                            @if($viewOnly)
                                <div class="text-gray-900">{{ $item['description'] ?? '-' }}</div>
                            @else
                                <input type="text"
